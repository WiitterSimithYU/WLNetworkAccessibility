//
//  BPNetWorkAccessibilityUnity.m
//  XXXXXX
//
//  Created by Weiliam on 2021/9/16.
//

#import "BPNetWorkAccessibilityUnity.h"
...
NSString * const BPYNetworkAccessibilityChangedNotification = @"BPYNetworkAccessibilityChangedNotification";

typedef NS_ENUM(NSInteger, BPYNetworkType) {
    BPYNetworkTypeUnknown ,
    BPYNetworkTypeOffline ,
    BPYNetworkTypeWiFi    ,
    BPYNetworkTypeCellularData ,
};


@interface BPNetWorkAccessibilityUnity ()
{
    SCNetworkReachabilityRef _reachabilityRef;
    CTCellularData *_cellularData;
    NSMutableArray *_becomeActiveCallbacks;
    BPYNetworkAccessibleState _previousState;
...
@implementation BPNetWorkAccessibilityUnity
...
+ (BPNetWorkAccessibilityUnity *)sharedInstance {
    static BPNetWorkAccessibilityUnity * instance = nil;
...
- (void)monitorNetworkAccessibleStateWithCompletionBlock:(void (^)(BPYNetworkAccessibleState))block {
...
- (void)setupNetworkAccessibility {
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue < 10.0 || [self isSimulator]) {
        ...
        [self notiWithAccessibleState:BPYNetworkAccessible];
        return;
    }
    ...
    BOOL firstRun = ({
        static NSString * RUN_FLAG = @"BPYNetworkAccessibilityRunFlag";
...
- (void)cleanNetworkAccessibility {
    ...
    _previousState = BPYNetworkChecking;
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
    BPNetWorkAccessibilityUnity *networkAccessibility = (__bridge BPNetWorkAccessibilityUnity *)info;
    if (![networkAccessibility isKindOfClass: [BPNetWorkAccessibilityUnity class]]) {
        return;
    }
...
//         **/
//        return [self notiWithAccessibleState:BPYNetworkAccessible];
//    }
    ...
    switch (state) {
        case kCTCellularDataRestricted: {// 系统 API 返回 无蜂窝数据访问权限
            ...
            [self getCurrentNetworkType:^(BPYNetworkType type) {
                /*  若用户是通过蜂窝数据 或 WLAN 上网，走到这里来 说明权限被关闭**/
                
                if (type == BPYNetworkTypeCellularData || type == BPYNetworkTypeWiFi) {
                    [self notiWithAccessibleState:BPYNetworkRestricted];
                } else {  // 可能开了飞行模式，无法判断
                    [self notiWithAccessibleState:BPYNetworkUnknown];
                }
            }];
...
- (void)getCurrentNetworkType:(void(^)(BPYNetworkType))block {
    if ([self isWiFiEnable]) {
        return block(BPYNetworkTypeWiFi);
    }
    BPYNetworkType type = [self getNetworkTypeFromStatusBar];
    if (type == BPYNetworkTypeWiFi) { // 这时候从状态栏拿到的是 Wi-Fi 说明状态栏没有刷新，延迟一会再获取
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
- (BPYNetworkType)getNetworkTypeFromStatusBar {
    NSInteger type = 0;
    @try {
    ...
    #pragma clang diagnostic pop
        ...
        if (isModernStatusBar) { // 在 iPhone X 上 statusBar 属于 UIStatusBar_Modern ，需要特殊处理
            ...
            return  wifiEnable     ? BPYNetworkTypeWiFi :
                    cellularEnable ? BPYNetworkTypeCellularData : BPYNetworkTypeOffline;
        } else { // 传统的 statusBar
            ...
            return type == 0 ? BPYNetworkTypeOffline :
                   type == 5 ? BPYNetworkTypeWiFi    : BPYNetworkTypeCellularData;
        }
...
- (void)notiWithAccessibleState:(BPYNetworkAccessibleState)state {
    if (_automaticallyAlert) {
        if (state == BPYNetworkRestricted) {
    ...
    if (state != _previousState) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:BPYNetworkAccessibilityChangedNotification object:nil];
